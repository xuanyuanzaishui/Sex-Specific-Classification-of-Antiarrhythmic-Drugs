function out = dydt_Grandi(t,statevar,Id,p,c,flag)
if ~exist('flag','var') || flag 
    flag = 1;
    statevar = num2cell(statevar);    
end 

[m, h, j, d, f, fcaBj, fcaBsl, xtos, ytos, xtof, ytof, xkr, xks, RyRr, RyRo, RyRi, NaBj, NaBsl, TnCL, TnCHc, ...
    TnCHm, CaM, Myoc, Myom, SRB, SLLj, SLLsl, SLHj, SLHsl, Csqnb, Ca_sr, Naj, Nasl, Nai, Ki, Caj, Casl, ...
    Cai, V, rtos, ICajuncinto] = deal(statevar{:}) ;%%%输入参数修改

y0x=[m; h; j; d; f; fcaBj; fcaBsl; xtos; ytos; xtof; ytof; xkr; xks; RyRr; RyRo; RyRi; NaBj; NaBsl; TnCL; TnCHc; ...
    TnCHm; CaM; Myoc; Myom; SRB; SLLj; SLLsl; SLHj; SLHsl; Csqnb; Ca_sr; Naj; Nasl; Nai; Ki; Caj; Casl; ...
    Cai; V; rtos; ICajuncinto];
%%%%%%%%%%%%%%%%%%%%%%%%%
...
Temp = p.T;     % [K]        (p.T)
FoRT = Frdy/R/Temp;
Cmem = 1.1e-10;   % [F] membrane capacitance 1.3810e-10;膜电容
Qpow = (Temp-310)/10;

% Cell geometry几何
cellLength = 100;     % cell length [um]113;%100 (p.L)
cellRadius = 10.25;   % cell radius [um]12;%10.25(p.rad)
...
%}
Vcell = pi*cellRadius^2*cellLength*1e-15;  % [L](p.Vcell)
Vmyo = 0.65*Vcell;
Vsr = 0.035*Vcell; 
Vsl = 0.02*Vcell; 
Vjunc = 1*0.0539*.01*Vcell; 
SAjunc = 20150*pi*2*junctionLength*junctionRadius;  % [um^2]
SAsl = pi*2*cellRadius*cellLength;          % [um^2] (p.Ageo)
...
Ko = p.Ko;   % Extracellular K   [mM][p.Ko]
Nao = p.Nao;  % Extracellular Na  [mM][p.Nao]
Cao = p.Cao;  % Extracellular Ca  [mM][p.Cao]
...
GNa=c.G.GNa*(1-0.1*AF);  % [mS/uF] [c.G.GNa ]
GNaB = 0.597e-3;      % [mS/uF]  [c.G.PNab ]
IbarNaK = c.G.Pnak*1.26;     % [uA/uF]
...
pCa = (1+0.5*ISO)*(1-0.5*AF)*c.G.GCaL;       % [cm/sec]
pK = (1+0.5*ISO)*(1-0.5*AF)*1.35e-7;        % [cm/sec]
Q10CaL = 1.8;       

% Ca transport parameters
IbarNCX = (1+0.4*AF)*c.G.Gncx; %[c.G.Gncx 36]     % [uA/uF]5.5 before - 9 in rabbit
KmCai = 3.59e-3;    % [mM]
KmCao = 1.3;        % [mM]
KmNai = 12.29;      % [mM]
KmNao = 87.5;       % [mM]
...
if markov_iks==0
gks_junc=1*(1+1*AF+2*ISO)*(c.G.GKs)*1; %[c.G.GKs_ 34]
gks_sl=1*(1+1*AF+2*ISO)*(c.G.GKs)*1;  %[c.G.GKs_ 34]
...
GtoFast=(1.0-0.7*AF)*c.G.Gto*1.0; %nS/pF maleckar; %human atrium[c.G.Gto 32]
...
%atrium
%equations for activation;
Gkur = 1*(1.0-0.5*AF)*(1+2*ISO)*c.G.GKur*(1+0.2*RA); %nS/pF maleckar[c.G.GKur 39] 0.045
...
I_ki =(1+1*AF)*c.G.GK1*sqrt(Ko/5.4)*kiss*(yxt(39)-ek);
...
J_SRleak = (c.G.Leak)*(1.0+0.25*AF)*5.348e-6*(yxt(31)-yxt(36));  % [mM/ms 46]
...
%电流
currents = [I_Na I_to I_Ca I_kr I_ks I_ki I_ncx I_Catot I_nak ...
    I_nabk I_cabk I_pca];
% ----- END EC COUPLING MODEL ---------------

Nydot = num2cell(ydot(1:41,:));

[dm, dh, dj, dd, df, dfcabj, dfcabsl, dxtos, dytos, dxtof, dytof, dxkr, dxks, ...
        dryrr, dryro, dryri, dnabj, dnabsl, dtncl, dtnchc, dtnchm, dcam, ...
        dmyoc, dmyom, dsrb,dsllj, dsllsl, dslhj, dslhsl, dcsqnb, dca_sr, ...
        dnaj, dnasl, dnai, dki, dcaj, dcasl, dcai, dv, drtos, dicajuncinto]= deal(Nydot{1:41,:}) ;

%%%%输出修改
if flag
    out=[dm; dh; dj; dd; df; dfcabj; dfcabsl; dxtos; dytos; dxtof; dytof; dxkr; dxks; ...
        dryrr; dryro; dryri; dnabj; dnabsl; dtncl; dtnchc; dtnchm; dcam; ...
        dmyoc; dmyom; dsrb;dsllj; dsllsl; dslhj; dslhsl; dcsqnb; dca_sr; ...
        dnaj; dnasl; dnai; dki; dcaj; dcasl; dcai; dv; drtos; dicajuncinto];
else
